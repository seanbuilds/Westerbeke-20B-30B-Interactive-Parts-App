...
export function parseSmokeArgs(argv, env = {}) {
  ...
  const defaultDir = process.env.GROK_SANDBOX === "1" || process.cwd() === "/workspace" ? "/workspace/screenshots" : `${process.cwd()}/screenshots`;
  return {
    url: positional[0] || "http://127.0.0.1:8080/",
    outPng: positional[1] || `${defaultDir}/app-builder-preview.png`,
    baseline,
  };
}
